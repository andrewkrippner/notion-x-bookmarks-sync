...
const bookmarksDb = worker.database("bookmarks", {
  ...
  schema: {
    properties: {
      ...
      Handle: Schema.richText(),
      "Author URL": Schema.url(),
      URL: Schema.url(),
      Posted: Schema.date(),
      Media: Schema.checkbox(),
      Likes: Schema.number(),
      Reposts: Schema.number(),
      Replies: Schema.number(),
      Quotes: Schema.number(),
      Impressions: Schema.number(),
      "Post ID": Schema.richText(),
    },
  },
});
...
function toChange(tweet: BookmarkTweet) {
  ...
  return {
    ...
    properties: {
      Post: Builder.title(summarizeTitle(tweet.text)),
      Type: Builder.select(type),
      Author: Builder.richText(tweet.authorName),
      Handle: Builder.richText(`@${tweet.authorUsername}`),
      "Author URL": Builder.url(`https://x.com/${tweet.authorUsername}`),
      URL: Builder.url(tweet.url),
      Posted: Builder.dateTime(toLocalISOString(tweet.createdAt, TIMEZONE), TIMEZONE),
      Media: Builder.checkbox(tweet.mediaUrls.length > 0),
      Likes: Builder.number(tweet.likes),
      Reposts: Builder.number(tweet.reposts),
      Replies: Builder.number(tweet.replies),
      Quotes: Builder.number(tweet.quotes),
      Impressions: Builder.number(tweet.impressions),
      "Post ID": Builder.richText(tweet.id),
    },
  };
}
